...
#include <LBFGS.h>
#include <LBFGSB.h>
double Solver::operator()(Eigen::VectorXd positions, Eigen::VectorXd & grad)
{
...
#pragma omp parallel for
	for (int p = 0; p < this->Atoms->NAtoms; p++)
	{
		...
		A3D_ELEM(v, Z0, Y0, X0) += 1.0;//v000;
		A3D_ELEM(v, Z0, Y0, X1) += 1.0;//v001;
		A3D_ELEM(v, Z0, Y1, X0) += 1.0;//v010;
		A3D_ELEM(v, Z0, Y1, X1) += 1.0;//v011;

		A3D_ELEM(v, Z1, Y0, X0) += 1.0;//v100;
		A3D_ELEM(v, Z1, Y0, X1) += 1.0;//v101;
		A3D_ELEM(v, Z1, Y1, X0) += 1.0;//v110;
		A3D_ELEM(v, Z1, Y1, X1) += 1.0;//v111;
	}
...
void Solver::run()
{
	...
	Eigen::VectorXd lb = Eigen::VectorXd::Constant(Atoms->NAtoms * 3, 0.0);
	Eigen::VectorXd ub = Eigen::VectorXd::Constant(Atoms->NAtoms * 3, 0.0);
	Eigen::VectorXd positions = Eigen::VectorXd::Zero(Atoms->NAtoms*3);
	for (size_t i = 0; i < Atoms->NAtoms; i++)
	{
		positions[i * 3] = Atoms->AtomPositions[i].x;
		positions[i * 3 + 1] = Atoms->AtomPositions[i].y;
		positions[i * 3 + 2] = Atoms->AtomPositions[i].z;
		ub[i * 3] = Dims.x - 1;
		ub[i * 3 + 1] = Dims.y - 1;
		ub[i * 3 + 2] = Dims.z - 1;
	}
	...
	double fx;
	int niter = solver.minimize(*this, positions, fx, lb, ub);
