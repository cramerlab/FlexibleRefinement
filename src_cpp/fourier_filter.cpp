...
void FourierFilter::apply(MultidimArray<DOUBLE> &img)
{
    ...
    if (maskFn != "")
        if (do_generate_3dmask==0 && MULTIDIM_SIZE(img)>1024*1024)
            REPORT_ERROR("Cannot save 2D mask with xdim*ydim  > 1M");
        else
        {
            MRCImage<int> I;
			MRCImage<DOUBLE> D;
            if ( XSIZE(maskFourier) !=0 )
            {
                ...
                I.writeAs<__int16>(maskFn);
            }
            else if (XSIZE(maskFourierd)!=0)
            {
                D.setData(maskFourierd);
                D.write(maskFn);
            }
            else
                REPORT_ERROR("Cannot save  mask file");
...
DOUBLE FourierFilter::maskValue(const Matrix1D<DOUBLE> &w)
{
   ...
   //     break;
    ...
    //    break;
    default:
        REPORT_ERROR("Unknown mask type");
    }
    return 0;
}
