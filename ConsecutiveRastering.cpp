...
int main(int argc, char** argv) {
	...
	bool weighting = true;
	//int N = 600000;
	int N_series[11] = { 1000, 10000, 20000, 30000, 50000, 100000, 200000, 500000, 600000, 800000, 1000000 };
	float Super_series[3] = { 1, 2, 4 };
	float3 *angles;
	...
	for each (float super in Super_series)
	{
		for each (int  N in N_series)
		{
			...
			FileName outdir = "D:\\EMD\\9233\\Consecutive_Rastering\\";
			...
			for (size_t i = 0; i < 10; i++)
			{
				...
				writeFSC(im()*mask(), rastered*mask(), std::string(outdir) + std::to_string(i) + "_fsc_masked.star");
				writeFSC(im(), rastered, std::string(outdir) + std::to_string(i) + "_fsc.star");
				writeProjectionsToDisk(Atoms, angles, numProj, super, dims, outdir + std::to_string(i) + "_proj.mrc");
